...
def create_intermediate_ca(args, logger):
    ...
    cert_builder = cert_builder.add_extension(
        x509.SubjectKeyIdentifier.from_public_key(int_key.public_key()), critical=False
    )
    ...
    logger.info(f"Intermediate CA certificate signed. Serial: {hex(int_cert.serial_number)}")
    ...
    create_policy_file(out_dir, args.subject, int_cert.serial_number, args.validity_days, args.key_type, args.key_size,
...
def issue_end_entity_cert(args, logger):
    ...
    logger.info(f"Certificate issued. Serial: {hex(leaf_cert.serial_number)}")

    out_dir = Path(args.out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)

    cn_attrs = subject_name.get_attributes_for_oid(NameOID.COMMON_NAME)
    base_name = cn_attrs[0].value if cn_attrs else str(serial_number)[-8:]
    ...
    logger.info(f"AUDIT: Issued {args.template} cert for {subject_name.rfc4514_string()}")
